...
$(LINUX_HEADERS): $(LINUX_DIRECTORY_STAMP)
	cd $(LINUX_DIRECTORY) && make allyesconfig && make INSTALL_HDR_PATH="$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)" headers_install
...
$(LIBXXF86VM): $(LIBXXF86VM_DIRECTORY_STAMP) $(PKG_CONFIG) $(XF86VIDMODEPROTO)
	cd $(LIBXXF86VM_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" ./configure --host=$(TOOLCHAIN_TARGET) --target=$(TOOLCHAIN_TARGET) --prefix=$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)
...
$(LIBXRANDR): $(LIBXRANDR_DIRECTORY_STAMP) $(PKG_CONFIG) $(LIBXEXT) $(RANDRPROTO) $(LIBXRENDER)
	cd $(LIBXRANDR_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" ./configure --host=$(TOOLCHAIN_TARGET) --target=$(TOOLCHAIN_TARGET) --prefix=$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)
...
$(LIBXRENDER): $(LIBXRENDER_DIRECTORY_STAMP) $(PKG_CONFIG) $(LIBXEXT) $(RENDERPROTO)
	cd $(LIBXRENDER_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" ./configure --host=$(TOOLCHAIN_TARGET) --target=$(TOOLCHAIN_TARGET) --prefix=$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)
...
$(LIBX11): $(LIBX11_DIRECTORY_STAMP) $(PKG_CONFIG) $(XTRANS) $(KBPROTO) $(INPUTPROTO) $(XF86BIGFONTPROTO) $(XEXTPROTO) $(LIBXCB)
	cd $(LIBX11_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" ./configure --host=$(TOOLCHAIN_TARGET) --target=$(TOOLCHAIN_TARGET) --prefix=$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)
...
$(LIBXEXT): $(LIBXEXT_DIRECTORY_STAMP) $(PKG_CONFIG) $(XEXTPROTO) $(LIBX11)
	cd $(LIBXEXT_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" ./configure --host=$(TOOLCHAIN_TARGET) --target=$(TOOLCHAIN_TARGET) --prefix=$(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET)
...
$(GLIB): $(GLIB_DIRECTORY_STAMP) $(PKG_CONFIG) $(MESON_CROSS_TXT)
	...
	#cd $(GLIB_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" meson setup build -Dcpp_link_args='-lrt -lpthread -ldl -lresolv' -Dc_link_args='-lrt -lpthread -ldl -lresolv' --prefix $(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET) --cross-file $(TOOLCHAIN_TARGET).txt
...
$(PULSEAUDIO): $(PULSEAUDIO_DIRECTORY_STAMP) $(PKG_CONFIG) $(GLIB) $(LIBSNDFILE) $(LIBTOOL) $(ALSA_LIB) $(MESON_CROSS_TXT)
	...
	#cd $(PULSEAUDIO_DIRECTORY) && PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH" meson setup build -Dc_link_args='-ldl -lm -lrt -lpthread -lsndfile' -Ddatabase=simple -Dx11=disabled -Dtests=false --prefix $(TOOLCHAIN_DIRECTORY)/$(TOOLCHAIN_TARGET) --cross-file $(TOOLCHAIN_TARGET).txt
...
all: $(GCC) $(LIBX11) $(LIBXEXT) $(PULSEAUDIO) $(LIBDRM) $(MESA) $(WAYLAND) $(LIBXKBCOMMON)
