...
TOOLCHAIN_ENV_SCRIPT=$(TOOLCHAIN_DIRECTORY).env.sh
...
$(TOOLCHAIN_ENV_SCRIPT) :
	mkdir -m 755 -p $(TOOLCHAIN_DIRECTORY)
	echo 'PATH="$(TOOLCHAIN_DIRECTORY)/bin:$$PATH"' > $@.tmp
	chmod 755 $@.tmp
	mv $@.tmp $@

all: $(GCC) $(LIBX11) $(LIBXI) $(LIBXEXT) $(PULSEAUDIO) $(LIBDRM) $(MESA) $(WAYLAND) $(LIBXKBCOMMON) $(TOOLCHAIN_ENV_SCRIPT)
